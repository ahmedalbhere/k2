...
async function barberSignup() {
    ...
    if (!name || !phone || !password || !confirmPassword || !address || !location) {
            ...
            barberCard.innerHTML = `
                <div class="barber-info">
                    ...
                    <div class="barber-details">
                        <div>العنوان: ${barber.address || 'غير متوفر'}</div>
                        <div>الموقع: <a href="${barber.location}" target="_blank">عرض على الخريطة</a></div>
...
async function bookAppointment(barberId, barberName) {
    ...
    if (barberSnapshot.exists()) {
        ...
        const queue = barberData.queue || {};
        
        // التحقق من وجود حجز سابق لهذا العميل عند هذا الحلاق
        const existingBooking = Object.values(queue).find(
            booking => booking.clientId === currentUser.id
        );
        
        if (existingBooking) {
            alert('لديك حجز بالفعل عند هذا الحلاق');
            return;
        }
    }
    
    // التحقق من وجود أي حجز للعميل عند أي حلاق
    if (currentUser.booking) {
        alert('لديك حجز نشط بالفعل، يرجى إلغاء الحجز الحالي قبل حجز جديد');
        return;
    }
